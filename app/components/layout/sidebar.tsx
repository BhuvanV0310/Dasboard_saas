...
const menu: MenuItem[] = [
  ...
  // Analysis Dashboard (admin and user, with branches dropdown)
  {
    label: "Analytics",
    roles: ["admin", "user"],
    icon: <DashboardIcon />,
...
function Sidebar({ role }: SidebarProps) {
  ...
  const csvBranches: DropdownItem[] = (uploads || [])
    .filter((upload: any) => upload.status === 'ACTIVE')
    .map((upload: { id: string; filename: string }) => ({
      label: upload.filename,
      href: `/dashboard/analytics/csv/${upload.id}`,
      icon: <BranchIcon />,
    }));

  // Inject dynamic dropdown into Analytics menu
  const dynamicMenu = menu.map((item) => {
    if (item.label === "Analytics") {
      return {
        ...item,
        dropdown: csvBranches.map((branch) => ({
          ...branch,
          label: `${branch.label.replace(/\.csv$/i, '')} Branch`,
        })),
      };
    }
    return item;
  ...
  return (
    <aside className="w-72 h-screen bg-gradient-to-br from-slate-900 via-blue-900 to-slate-900 text-white shadow-2xl flex flex-col overflow-hidden fixed left-0 top-0 z-50">
      {/* Background Pattern */}
