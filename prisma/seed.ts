...
async function main() {
  ...
  // Create test admin (demo) to match frontend demo credentials
  const hashedTestAdminPassword = await bcrypt.hash('TestAdmin123!', 10);
  ...
  console.log('✅ Demo admin created:', testAdmin.email);

  // Create an additional test user for convenience
  const hashedTester2Password = await bcrypt.hash('TestUser123!', 10);
  const tester2 = await prisma.user.upsert({
    where: { email: 'tester2@example.com' },
    update: {},
    create: {
      email: 'tester2@example.com',
      password: hashedTester2Password,
      name: 'Tester Two',
      role: 'CUSTOMER',
    },
  });
  console.log('✅ Test user created:', tester2.email);
